...
void MyIIC_Init(void){
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);//PB3,4被jtag占用
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE); //禁用后可做普通gpio
...
//void MyIIC_Start(void)
//{
//	MyIIC_W_SDA(1);							//释放SDA，确保SDA为高电平
//	MyIIC_W_SCL(1);							//释放SCL，确保SCL为高电平
//	MyIIC_W_SDA(0);							//在SCL高电平期间，拉低SDA，产生起始信号
//	MyIIC_W_SCL(0);							//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
//}
...
//void MyIIC_Stop(void)
//{
//	MyIIC_W_SDA(0);							//拉低SDA，确保SDA为低电平
//	MyIIC_W_SCL(1);							//释放SCL，使SCL呈现高电平
//	MyIIC_W_SDA(1);							//在SCL高电平期间，释放SDA，产生终止信号
//}
...
//		MyIIC_W_SDA(!!(Byte & (0x80 >> i)));//使用掩码的方式取出Byte的指定一位数据并写入到SDA线
//		MyIIC_W_SCL(1);						//释放SCL，从机在SCL高电平期间读取SDA
//		MyIIC_W_SCL(0);						//拉低SCL，主机开始发送下一位数据
//	}
//}
...
//	{
//		MyIIC_W_SCL(1);						//释放SCL，主机机在SCL高电平期间读取SDA
//		if (MyIIC_R_SDA()){Byte |= (0x80 >> i);}	//读取SDA数据，并存储到Byte变量
//													//当SDA为1时，置变量指定位为1，当SDA为0时，不做处理，指定位为默认的初值0
//		MyIIC_W_SCL(0);						//拉低SCL，从机在SCL低电平期间写入SDA
//	}
...
//{
//	MyIIC_W_SDA(AckBit);					//主机把应答位数据放到SDA线
//	MyIIC_W_SCL(1);							//释放SCL，从机在SCL高电平期间，读取应答位
//	MyIIC_W_SCL(0);							//拉低SCL，开始下一个时序模块
//}
